...
The Tokyocoin Core daemon can be configured to act as a trusted "border
router", implementing the tokyocoin wire protocol and relay, making
...
The ZeroMQ feature in Tokyocoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building tokyocoind:
...
For instance:
    ...
    $ tokyocoind -zmqpubhashtx=tcp://127.0.0.1:21445 \
               -zmqpubhashtx=tcp://192.168.1.2:21445 \
               -zmqpubrawtx=ipc:///tmp/tokyocoind.tx.raw \
...
These options can also be provided in tokyocoin.conf.
...
From the perspective of tokyocoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into tokyocoind directly. Furthermore, no information is
...
using. Tokyocoind appends an up-counting sequence number to each
