...
uint256 SendCoins(CWallet& wallet, SendCoinsDialog& sendCoinsDialog, const CTxDestination& address, CAmount amount, bool rbf)
{
    ...
    entry->findChild<TokyocoinAmountField*>("payAmount")->setValue(amount);
...
//
//     QT_QPA_PLATFORM=xcb     src/qt/test/test_tokyocoin-qt  # Linux
//     QT_QPA_PLATFORM=windows src/qt/test/test_tokyocoin-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   src/qt/test/test_tokyocoin-qt  # macOS
void TestGUI(interfaces::Node& node)
{
...

    {
        ...
        QString balanceComparison = TokyocoinUnits::formatWithUnit(unit, balance, false, TokyocoinUnits::SeparatorStyle::ALWAYS);
    ...
    // Amount input
    TokyocoinAmountField* amountInput = receiveCoinsDialog.findChild<TokyocoinAmountField*>("reqAmount");
    ...
    for (QWidget* widget : QApplication::topLevelWidgets()) {
        if (widget->inherits("ReceiveRequestDialog")) {
            ...
            QCOMPARE(uri.count("tokyocoin:"), 2);
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        QWARN("Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
              "with 'QT_QPA_PLATFORM=cocoa test_tokyocoin-qt' on mac, or else use a linux or windows build.");
        return;
    }
#endif
    TestGUI(m_node);
