...
#include <qt/tokyocoinaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Tokyocoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new TokyocoinAddressEntryValidator(parent));
    widget->setCheckValidator(new TokyocoinAddressCheckValidator(parent));
}

bool parseTokyocoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no tokyocoin: URI
    if(!uri.isValid() || uri.scheme() != QString("tokyocoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!TokyocoinUnits::parse(TokyocoinUnits::TKC, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseTokyocoinURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseTokyocoinURI(uriInstance, out);
}

QString formatTokyocoinURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("tokyocoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(TokyocoinUnits::format(TokyocoinUnits::TKC, info.amount, false, TokyocoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openTokyocoinConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", TOKYOCOIN_CONF_FILENAME));
    ...
    /* Open tokyocoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Tokyocoin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Tokyocoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Tokyocoin (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Tokyocoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "tokyocoin.desktop";
    return GetAutostartDir() / strprintf("tokyocoin-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a tokyocoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Tokyocoin\n";
        else
            optionFile << strprintf("Name=Tokyocoin (%s)\n", chain);
