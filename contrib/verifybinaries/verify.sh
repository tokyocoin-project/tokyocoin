...
###   tokyocoin.de and tokyocoin.org and compares them.
...
WORKINGDIR="/tmp/tokyocoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://tokyocoin.de"
HOST2="https://tokyocoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="tokyocoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'tokyocoin-',
...
if ! WGETOUT=$(wget -N -O "$SIGNATUREFILENAME.2" "$HOST2$BASEDIR$SIGNATUREFILENAME" 2>&1); then
   echo "tokyocoin.org failed to provide signature file, but tokyocoin.de did?"
   echo "wget output:"
   # shellcheck disable=SC2001
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "tokyocoin.org and tokyocoin.de signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Tokyocoin Core binary release signing key installed?"
   fi
